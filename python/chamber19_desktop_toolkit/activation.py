...
async def activate_machine(req: ActivationPayload, request: Request):
    ...
    if datetime.utcnow() > expires_at:
        del pins_db[pin]
        error = "PIN expired"
        log_audit("activate", pin, "", client_ip, hardware, False, error)
        raise HTTPException(status_code=400, detail=error)
    ...
    # Log success
    log_audit("activate", pin, "", client_ip, hardware, True)
...
async def validate_token(hardware_fingerprint: str, token: str, request: Request):
    ...
    if age_days > TOKEN_EXPIRY_DAYS:
        ...
        log_audit("validate_token", None, "", client_ip, hardware_fingerprint, False, error)
        raise HTTPException(status_code=401, detail=error)
    
    # Log success
    log_audit("validate_token", None, "", client_ip, hardware_fingerprint, True)
...
async def revoke_machine(hardware_fingerprint: str):
    ...
    if hardware_fingerprint in machines_db:
        machine = machines_db[hardware_fingerprint]
        del machines_db[hardware_fingerprint]
        logger.info(f"Machine revoked: {hardware_fingerprint}")
